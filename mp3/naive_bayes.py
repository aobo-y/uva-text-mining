...
class NaiveBayes:
  def __init__(self, corpus, vocab, delta = 0.1):
    ...
    self.build_lang_models(corpus, vocab, delta)

  def build_lang_models(self, reviews, vocab, delta = 0.1):
    pos_counts = Counter()
    neg_counts = Counter()
    ...
    self.pos_model = LanguageModel(pos_counts, vocab, delta)
    self.neg_model = LanguageModel(neg_counts, vocab, delta)
  ...
  def f_value(self, review):
    return self.log_ratio + sum([
      math.log(self.pos_model.probs[w]) - math.log(self.neg_model.probs[w])
      for w in review.features
    ])
